...
class Bdev_logger(ABC):
    ...
    def __create_config_for_fio(self) -> ConfigParser:
        ...
        file_name_param = self.__get_file_name_param()
        ...
        for rw in [i.strip() for i in self.settings["global"]["rw"].split(',')]:
            ...
            fio_file[section_name]["write_bw_log"] = f"{self.__logs_dir_path}/{self.settings['global']['bs']}-{rw}-{self.__get_mode()}.results"
            fio_file[section_name]["write_iops_log"] = f"{self.__logs_dir_path}/{self.settings['global']['bs']}-{rw}-{self.__get_mode()}.results"
            fio_file[section_name]["write_lat_log"] = f"{self.__logs_dir_path}/{self.settings['global']['bs']}-{rw}-{self.__get_mode()}.results"
